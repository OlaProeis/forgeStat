...
impl App {
    pub(super) fn render_help_overlay(&self, frame: &mut Frame) {
        ...
        // Render each column
        let col1 = Paragraph::new(self.build_global_shortcuts_text()).block(
            Block::default()
        ...
        let col2 = Paragraph::new(self.build_navigation_shortcuts_text()).block(
            Block::default()
                .borders(Borders::RIGHT)
                .border_style(Style::default().fg(self.theme.help_border_color())),
        );
        ...
        let col3 = Paragraph::new(self.build_zoom_mouse_shortcuts_text()).block(Block::default());
        frame.render_widget(col3, col3_area);

        // Render command palette section
        let cmd_text = self.build_command_palette_text();
        let cmd_para = Paragraph::new(cmd_text).block(
            Block::default()
                .borders(Borders::TOP)
                .border_style(Style::default().fg(self.theme.help_border_color())),
        );
    ...
    fn build_global_shortcuts_text(&self) -> Text<'static> {
        ...
        let lines = vec![
            Line::from(vec![Span::styled(
                "Global",
                Style::default().fg(self.theme.help_title_color()).bold(),
            )]),
            Line::from(vec![
                Span::styled(
                    "?",
    ...
    fn build_search_filter_text(&self) -> Text<'static> {
        ...
        let lines = vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    "           Open search (Issues, Contributors, Releases)",
                    Style::default().fg(self.theme.text_primary_color()),
                ),
                Span::styled(
                    "   Esc / c",
            ...
            Line::from(vec![
                ...
                Span::styled(
                    "           Cycle label filter (Issues only)",
                    Style::default().fg(self.theme.text_primary_color()),
                ),
                Span::styled(
                    "                p",
                ...
                Span::styled(
                    "         Cycle pre-release filter (Releases)",
                    Style::default().fg(self.theme.text_primary_color()),
                ),
            ]),
        ];
