...
impl App {
    pub(super) fn render_mini_map_overlay(&self, frame: &mut Frame) {
        ...
        let all_constraints: Vec<Constraint> = health_constraints.into_iter().chain(panel_constraints).collect();
        ...
        if let Some(ref health) = self.health_score {
            ...
            let health_text = Line::from(vec![
                Span::styled(" Health Score: ", Style::default().fg(self.theme.text_secondary_color())),
                Span::styled("Computing...", Style::default().fg(self.theme.text_secondary_color())),
            ]);
            frame.render_widget(Paragraph::new(health_text), areas[0]);
        }
        ...
        let stars_text = vec![
            Line::from(vec![
                Span::styled("1. ★ Stars", Style::default().fg(self.theme.text_primary_color()).bold()),
                Span::styled(
        ...
        let oldest_issue = snap.oldest_issue_age_days().map(|d| format!("{}d", d)).unwrap_or_else(|| "N/A".to_string());
        let issues_text = vec![
            Line::from(vec![
                Span::styled("2. Issues", Style::default().fg(self.theme.text_primary_color()).bold()),
                Span::styled(
            ...
            Line::from(vec![
                Span::styled("   Labels: ", Style::default().fg(self.theme.text_secondary_color())),
                Span::styled(
                ...
                Span::styled(" | Oldest: ", Style::default().fg(self.theme.text_secondary_color())),
                Span::styled(oldest_issue, Style::default().fg(self.theme.indicator_warning_color())),
            ]),
            Line::from(""),
        ];
        ...
        let merge_time = pr.avg_time_to_merge_hours.map(|h| format!("{:.1}h", h)).unwrap_or_else(|| "N/A".to_string());
        let prs_text = vec![
            Line::from(vec![
                Span::styled("3. Pull Requests", Style::default().fg(self.theme.text_primary_color()).bold()),
                Span::styled(
            ...
            Line::from(vec![
                ...
                Span::styled(" | Avg: ", Style::default().fg(self.theme.text_secondary_color())),
        ...
        let top_contrib = contrib.top_contributors.first().map(|c| format!("{} ({} commits)", c.username, c.commit_count)).unwrap_or_else(|| "N/A".to_string());
        let contributors_text = vec![
            Line::from(vec![
                Span::styled("4. Contributors", Style::default().fg(self.theme.text_primary_color()).bold()),
                Span::styled(
            ...
            Line::from(vec![
                Span::styled("   Top: ", Style::default().fg(self.theme.text_secondary_color())),
                Span::styled(top_contrib, Style::default().fg(self.theme.text_primary_color())),
                Span::styled(" | 30d: ", Style::default().fg(self.theme.text_secondary_color())),
                Span::styled(new_contrib, Style::default().fg(self.theme.indicator_success_color())),
            ]),
            Line::from(""),
        ];
        ...
        let release_days = snap.days_since_last_release().map(|d| format!("{}d ago", d)).unwrap_or_else(|| "N/A".to_string());
        let avg_interval = snap.releases.first().and_then(|r| r.avg_interval).map(|a| format!("{:.0}d", a)).unwrap_or_else(|| "N/A".to_string());
        let release_count = snap.releases.len();
        let latest_release = snap.releases.first().map(|r| r.tag_name.clone()).unwrap_or_else(|| "None".to_string());
        let releases_text = vec![
            Line::from(vec![
                Span::styled("5. Releases", Style::default().fg(self.theme.text_primary_color()).bold()),
                Span::styled(
            ...
            Line::from(vec![
                ...
                Span::styled(latest_release, Style::default().fg(self.theme.text_primary_color())),
                Span::styled(" | ", Style::default().fg(self.theme.text_secondary_color())),
                Span::styled(release_days, Style::default().fg(self.theme.indicator_info_color())),
                Span::styled(" | Avg interval: ", Style::default().fg(self.theme.text_secondary_color())),
                Span::styled(avg_interval, Style::default().fg(self.theme.indicator_info_color())),
            ]),
            Line::from(""),
        ];
        ...
        let issues_this_week = vel.issues_weekly.last().map(|w| format!("+{}/-{}", w.opened, w.closed)).unwrap_or_else(|| "N/A".to_string());
        let prs_this_week = vel.prs_weekly.last().map(|w| format!("+{}/-{}", w.opened, w.closed)).unwrap_or_else(|| "N/A".to_string());
        let velocity_text = vec![
            Line::from(vec![
                Span::styled("6. Velocity (8 weeks)", Style::default().fg(self.theme.text_primary_color()).bold()),
            ]),
            Line::from(vec![
                Span::styled("   Issues: ", Style::default().fg(self.theme.text_secondary_color())),
                Span::styled(issues_this_week, Style::default().fg(self.theme.indicator_info_color())),
                Span::styled(" | PRs: ", Style::default().fg(self.theme.text_secondary_color())),
                Span::styled(prs_this_week, Style::default().fg(self.theme.indicator_info_color())),
            ]),
            Line::from(""),
        ];
        ...
        let security_text = if let Some(ref sec) = snap.security_alerts {
            vec![
                Line::from(vec![
                    Span::styled("7. Security", Style::default().fg(self.theme.text_primary_color()).bold()),
                    Span::styled(
                        format!(" — Total: {}", sec.total_open),
                        Style::default().fg(if sec.total_open > 0 { self.theme.severity_critical_color() } else { self.theme.indicator_success_color() }),
                    ),
                ]),
                Line::from(vec![
                    Span::styled("   C: ", Style::default().fg(self.theme.text_secondary_color())),
                    Span::styled(
                        format!("{}", sec.critical_count),
                        self.severity_style_for_mini_map(sec.critical_count, self.theme.severity_critical_color()),
                    ),
                    Span::styled(" | H: ", Style::default().fg(self.theme.text_secondary_color())),
                    Span::styled(
                        format!("{}", sec.high_count),
                        self.severity_style_for_mini_map(sec.high_count, self.theme.severity_high_color()),
                    ),
                    Span::styled(" | M: ", Style::default().fg(self.theme.text_secondary_color())),
                    Span::styled(
                        format!("{}", sec.medium_count),
                        self.severity_style_for_mini_map(sec.medium_count, self.theme.severity_medium_color()),
                    ),
                    Span::styled(" | L: ", Style::default().fg(self.theme.text_secondary_color())),
                    Span::styled(
                        format!("{}", sec.low_count),
                        self.severity_style_for_mini_map(sec.low_count, self.theme.severity_low_color()),
                    ),
                ]),
            ]
        } else {
            vec![
                Line::from(vec![
                    Span::styled("7. Security", Style::default().fg(self.theme.text_primary_color()).bold()),
                    Span::styled(" — Alerts unavailable", Style::default().fg(self.theme.text_secondary_color())),
                ]),
                Line::from(vec![
                    Span::styled("   (Dependabot disabled or no access)", Style::default().fg(self.theme.text_secondary_color())),
                ]),
            ]
        };
        ...
        let ci_text = if let Some(ref ci) = snap.ci_status {
            ...
            let (last_status_icon, last_status_color) = ci.recent_runs.first()
                .map(|run| {
            ...
            vec![
                Line::from(vec![
                    Span::styled("8. CI Status", Style::default().fg(self.theme.text_primary_color()).bold()),
                    Span::styled(
                ...
                Line::from(vec![
                    ...
                    Span::styled(last_status_icon, Style::default().fg(last_status_color).bold()),
                    Span::styled(
            ...
            vec![
                Line::from(vec![
                    Span::styled("8. CI Status", Style::default().fg(self.theme.text_primary_color()).bold()),
                    Span::styled(" — Actions unavailable", Style::default().fg(self.theme.text_secondary_color())),
                ]),
                Line::from(vec![
                    Span::styled("   (GitHub Actions disabled or no access)", Style::default().fg(self.theme.text_secondary_color())),
                ]),
            ]
        };
